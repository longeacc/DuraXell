    ...
    for doc in docs:
        for ent in doc["entities"]:
            ...
            if text not in annotations[entity]["category?"]:
                annotations[entity]["category?"][text] = {
                    "occurrences": 1,
                    "stems": list_stem,
                    "places": [place],
                }
    ...
    for entity in list(current_annotations.keys()):
        ...
        for text in current_annotations[entity]["category?"]:
            ...
            for tempo_text in tempo_annotations[entity]["category?"]:
                ...
                if (
                    dist_leven < dist
                    and not contain_digit(stems)
                    and not contain_digit(tempo_stems)
                    and len(tempo_stems) > 4
                ) or (dist_leven == 0):
...
def print_annotations_caracteristics(dict_annotations):
    ...
    for entity in entities:
        ...
        for category in categories:
            ...
            print(
                "   Total number of different annotations :",
                len(dict_annotations[entity]["category?"]),
            )
            print("\n")
