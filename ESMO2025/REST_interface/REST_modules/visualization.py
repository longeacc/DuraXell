...
def create_dg_results(df_results):
    if df_results.empty:
        ...
        dg_results = DataGrid(
            ...
            layout={"height": "50px", "width": "800px"},
            base_row_size=20,
        )
        return dg_results
    ...
    df_temp.loc[:, "recall_confidence_interval"] = df_temp.apply(
        lambda row: f"[{row['recall_conf_inter_down']}, {row['recall_conf_inter_up']}]",
        axis=1,
    )
    ...
    dg_results = DataGrid(
        ...
        column_widths={
            ...
            "FN": 50,
            "precision": 80,
            "precision_confidence_interval": 120,
            "recall": 80,
            "recall_confidence_interval": 120,
        },
        layout={"height": height, "width": "1120px"},
        base_row_size=20,
    )
    ...
    renderers = {
        ...
        "precision": BarRenderer(
            ...
            bar_color=ColorScale(min=0, max=1, scheme="cividis"),
...
def create_categories_donut(cat_infos, current_entity):
    ...
    fig = go.Figure(
        data=[
            go.Pie(
                labels=cat_infos["category"].tolist(),
    ...
    fig.update_layout(
        title="Categories distribution for the entity : " + str(current_entity),
        title_x=0.5,
    )
...
def visualize_category_selection(change, df, current_entity):
    ...
    df1 = DataGrid(
        df1,
        column_widths={"text": 800, "occurrences": 100},
        layout={"height": f"{height}px", "width": "990px"},
        base_row_size=25,
    )
    return df1
