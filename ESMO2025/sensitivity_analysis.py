...
def load_real_metrics(results_dir: Path) -> dict:
    ...
    if freq_file.exists():
        with open(freq_file, "r", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ...
                if ent: metrics_db.setdefault(ent, {})["Freq"] = float(row.get("Frequency", 0))
    ...
    if he_file.exists():
        with open(he_file, "r", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ent = row.get("Entity")
                if ent: metrics_db.setdefault(ent, {})["He"] = float(row.get("He_Score_Percent", 0))
        ...
        try:
            with open(te_file, "r", encoding="utf-8") as f:
                ...
                for ent, vals in data.items():
                    metrics_db.setdefault(ent, {})["Te"] = vals.get("templeability_score", 0)
    ...
    if risk_file.exists():
        with open(risk_file, "r", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ent = row.get("Entity")
                if ent: metrics_db.setdefault(ent, {})["R"] = float(row.get("R_Score", 0))
...
def run_sensitivity_analysis(metrics_data: dict, thresholds_to_vary: list = ["TE_MED", "TE_HIGH", "HE_HIGH", "RISK_HIGH", "YIELD_HIGH", "FEAS_NER", "DOMAIN_SHIFT_MAX", "LLM_NEC_HIGH"], perturbation_pct: list = [-0.2, -0.1, 0.1, 0.2]):
    ...
    for threshold_name in thresholds_to_vary:
        if threshold_name not in base_thresholds: continue
        ...
        for pct in perturbation_pct:
            ...
            results.append({
                "threshold": threshold_name, "perturbation": f"{pct*100:+.0f}%",
                "original_value": original_val, "new_value": new_val,
                "n_changes": changes, "changed_entities": ", ".join(changed_entities),
...
if __name__ == "__main__":
    ...
    if metrics_data:
        ...
        print("\n=== RESULTATS ===")
        for r in results:
            print(f"- {r['threshold']} ({r['perturbation']}): {r['new_value']:.2f} -> {r['n_changes']} changements ({r['robustness']:.2%})")
            if r['n_changes'] > 0: print(f"    Detail: {r['changed_entities']}")
