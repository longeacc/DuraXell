...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../"))) # Go up to DuraXELL root
...
class NERCascadeConnector:
    ...
    def _load_model(self):
        ...
        if not os.path.exists(os.path.join(effective_dir, "config.json")):
             ...
             subdirs = [os.path.join(self.model_dir, d) for d in os.listdir(self.model_dir) if d.startswith("checkpoint-")]
             if subdirs:
    ...
    def predict(self, text: str, entity_type: str) -> ExtractionResult:
        ...
        for idx, (label_id, (start, end)) in enumerate(zip(pred_ids, offsets)):
             ...
             if prefix == "B" or (current_entity and current_entity['type'] != tag):
                 ...
                 current_entity["text"] += text[current_entity["end_prev"]:end] # Handle spaces if any? Actually slice from orig text is better
        ...
        if entity_type == "Estrogen_receptor": target_tags = ["ER", "Estrogen_Receptor"]
        if entity_type == "HER2": target_tags = ["HER2", "CerbB2"]
