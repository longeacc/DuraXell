...
def bloc1_performance():
    ...
    data = {
        "Biomarqueur": ["ER", "ER", "ER", "Ki67", "Ki67", "Ki67", "Grade", "Grade", "Grade"],
        "Méthode": ["Règles", "ML_CRF", "LLM", "Règles", "ML_CRF", "LLM", "Règles", "ML_CRF", "LLM"],
...
def bloc2_explainability():
    ...
    data = {
        ...
        "Score_Explicabilité": [CompositeScorer.EXPLAINABILITY_SCORES["REGLES"], CompositeScorer.EXPLAINABILITY_SCORES["ML_CRF"], CompositeScorer.EXPLAINABILITY_SCORES["Transformer"], CompositeScorer.EXPLAINABILITY_SCORES["LLM"]]
    }
    df = pd.DataFrame(data)
...
def bloc4_pareto(df_perf, df_expl, df_energy):
    ...
    for _, row in df_perf.iterrows():
        ...
        energy_val = df_energy[df_energy["Méthode"] == row["Méthode"]]["Energie_kWh"].values[0] if row["Méthode"] in df_energy["Méthode"].values else 0.01
...
def bloc6_figures(df_perf, df_expl, df_energy, df_pareto):
    ...
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=df_pareto, x="Energie_kWh", y="F1_Score", hue="Méthode", style="Biomarqueur", s=100)
    plt.xscale("log")
...
def main():
    ...
    print("Toutes les métriques et figures ont été générées avec succès dans le dossier 'Results/'.")
