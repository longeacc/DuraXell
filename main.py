...
ALL_ENTITIES = [
    ...
    "HER2_status", "HER2_IHC", "Ki67", "HER2_FISH", "Genetic_mutation",
]
...
def cmd_extract(args):
    ...
    print(f"Result: {res.value} | Method: {res.method_used} | Confidence: {res.confidence} | Energy: {res.energy_kwh:.6f} kWh")
...
def cmd_extract_all(args):
    ...
    for ent in entities:
        ...
        print(f"  {ent:<25s} => {status:<20s} | {res.method_used:<12s} | conf={res.confidence:.2f} | E={res.energy_kwh:.6f} kWh")
...
def cmd_batch(args):
    ...
    for fpath in files:
        ...
        for ent in entities:
            ...
            if done % 50 == 0 or done == total:
                ...
                print(f"  [{done}/{total}] {elapsed:.1f}s - dernier: {fname} / {ent} => {res.value or '-'}")

    # Write CSV
    fieldnames = [
        "fichier", "entite", "valeur", "methode_utilisee", "confiance", "energie_kwh",
        "Te", "He", "Freq", "Feas", "DomainShift", "LLM_Necessity",
        "methode_recommandee", "justification",
    ]
    ...
    elapsed = time.time() - t0
    print("-" * 70)
    print(f"Terminé en {elapsed:.1f}s | {found}/{total} extractions trouvées ({100*found/total:.0f}%)")
...
def main():
    ...
    parser_extract = subparsers.add_parser("extract", help="Extract a single entity from a document text")
    parser_extract.add_argument("--doc", type=str, required=True, help="Text document")
    parser_extract.add_argument("--entity", type=str, required=True, help="Entity to extract (e.g. Estrogen_receptor)")
    parser_extract.set_defaults(func=cmd_extract)

    # Command: extract-all (all entities at once)
    parser_extract_all = subparsers.add_parser("extract-all", help="Extract ALL known entities from a document")
    parser_extract_all.add_argument("--doc", type=str, required=True, help="Text document")
    parser_extract_all.add_argument("--entities", type=str, default=None, help="Comma-separated entity list (default: all 7)")
    parser_extract_all.set_defaults(func=cmd_extract_all)

    # Command: batch (directory of files × entities)
    parser_batch = subparsers.add_parser("batch", help="Extract entities from all .txt files in a directory")
    parser_batch.add_argument("--input_dir", type=str, required=True, help="Directory containing .txt files")
    parser_batch.add_argument("--entities", type=str, default=None, help="Comma-separated entity list (default: all 7)")
    parser_batch.set_defaults(func=cmd_batch)

    # Command: metrics
    parser_metrics = subparsers.add_parser("metrics", help="Run ESMO2025 metric pipelines")
    parser_metrics.set_defaults(func=cmd_metrics)

    # Command: tree
    parser_tree = subparsers.add_parser("tree", help="Generate decision tree and visual representation")
    ...
    # Command: serve
    parser_serve = subparsers.add_parser("serve", help="Serve DuraXELL model locally via HTTP (mock)")
