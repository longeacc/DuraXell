...
with st.sidebar:
    ...
    local_path = st.text_input("Chemin du dossier de travail (Corpus BRAT)", 
    ...
    if "corpus" in st.session_state and st.session_state["corpus"]:
        ...
        st.write(f"Annotations: {sum(stats['count'] for stats in st.session_state['entity_stats'].values())}")
