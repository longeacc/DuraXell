...
class MetricsCalculator:
    def __init__(self):
        self.rules = {
            'Estrogen_receptor': re.compile(r'(?:RE|RO|ER|Estrogen)[\s:]*(\d+\s*%|positif|négatif|positive|negative|\+|\-)', re.IGNORECASE),
            'Progesterone_receptor': re.compile(r'(?:RP|PR|Progesterone)[\s:]*(\d+\s*%|positif|négatif|positive|negative|\+|\-)', re.IGNORECASE),
            'Ki67': re.compile(r'Ki[\-\s]?67[\s:]*(\d+\s*%)', re.IGNORECASE),
            'HER2_status': re.compile(r'HER[\-\s]?2[\s:]*(\d\+?|positif|négatif|équivoque|score)', re.IGNORECASE),
            'HER2_IHC': re.compile(r'HER[\-\s]?2[\s:]*(\d\+?|positif|négatif|équivoque|score)', re.IGNORECASE),
            'HER2_FISH': re.compile(r'FISH|amplifié', re.IGNORECASE),
            'Genetic_mutation': re.compile(r'mutation|variant|BRCA', re.IGNORECASE),
        }
        self.NEGATION_PATTERNS = [
            r"\baucun\b", r"\bsans\b", r"\bni\b", r"\bpas\b", r"\babsence\b"
        ]
        self.UNCERTAINTY_PATTERNS = [
            r"\bprobable\b", r"\bpossible\b", r"\bà confirmer\b", r"\ba confirmer\b", 
            r"\bsuspecté\b", r"\bsuspecte\b", r"\béquivoque\b", r"\bequivoque\b", 
            r"\bdiscuté\b", r"\bincertain\b", r"\bhypothèse\b", r"\?", 
            r"\bdiscordant\b", r"\bdiscordance\b"
        ]
        self.CONTRADICTION_PATTERNS = [
            r"\bcependant\b", r"\bmais\b", r"\bnéanmoins\b", r"\bau contraire\b", 
            r"\bmalgré\b", r"\btoutefois\b", r"\bà l'inverse\b"
        ]
    ...
    def compute_all_metrics(self, documents: List[Any], entity_type: str) -> Dict[str, float]:
        ...
        if not annotations:
            return {'Te': 0.0, 'He': 0.0, 'R': 0.0, 'Freq': 0.0, 'Yield': 0.0, 'Feas': 0.0, 'DomainShift': 0.0, 'LLM_Necessity': 0.0}
            
        values = [a.value.strip() for a in annotations if a.value]
        contexts = [a.context.lower().strip() if hasattr(a, 'context') and a.context else a.value.lower().strip() for a in annotations]
        ...
        if num_unique <= 1:
            ...
            entropy = -sum((p/len(normalized_patterns)) * math.log(p/len(normalized_patterns)) for p in counter.values())
            H_norm = entropy / math.log(num_unique)
            
        structure_consistency = 1.0 - H_norm
        bonus_semantic = 0.1 if any(c in p for p in set(normalized_patterns) for c in ["%", "+", "-", ">", "<"]) else 0.0
        ...
        r_raw = (negated / total_texts) * 0.2 + (uncertain / total_texts) * 0.5 + (contradictory / total_texts) * 1.0 if total_texts > 0 else 0.0
