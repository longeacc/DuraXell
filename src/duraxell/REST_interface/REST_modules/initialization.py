...
def initialize_globals():
    ...
    df = pd.DataFrame(
        ...
        columns=["entity", "category", "text", "occurrences", "stems", "places"],
    )
    df_tf_results = pd.DataFrame(
        [["entity1", "word1", "occurrences1", "tfidf1"]],
        columns=["entity", "word", "occurrences", "tfidf"],
    )
...
def initialize_widgets(ent_cat, current_entity, ban_words_entities, options):
    ...
    button_categorization = widgets.Button(
        button_style="primary", description="Categorization", icon="tasks"
    )
