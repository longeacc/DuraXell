            ...
            with open(os.path.join(path, file_name), newline="", encoding="utf-8") as file:
                ...
                for match in re.finditer(pattern, text):
                    ...
                    if fp:  # verification in other categories, if exist we add in 'FP'
                        ...
                        filtre2 = (
                            (df_other_categories["entity"] == current_entity)
...
def calculate_df_metrics(df_location_metrics):
    ...
    df_metrics = pd.DataFrame(
        metrics,
        columns=["category", "TP(corr)", "TP", "FP", "FN", "precision", "recall"],
    )
    df_metrics["precision"] = pd.to_numeric(df_metrics["precision"]).round(2)
    df_metrics["recall"] = pd.to_numeric(df_metrics["recall"]).round(2)
...
def create_grid_metrics_locations(df_metrics_locations, current_entity):
    ...
    dg_location_metrics = DataGrid(
        df_metrics_locations[["category", "result", "text", "file", "places"]],
        column_widths={
            "category": 200,
            "result": 80,
            "text": 650,
        ...
        layout={"height": "350px", "width": "1200px"},
        base_row_size=25,
        selection_mode="row",
    )
    ...
    renderers = {
        ...
        "text": TextRenderer(horizontal_alignment="left"),
    }
...
def background_color_tp(cell):
    ...
    # pre
        ...
        cell.column == 0
        and cell.value == cell.metadata.data["('corrected highlights', 'TP(corr)')"]
    ):  # TP
        return "orange"
    elif (
        cell.column == 1
        and cell.value == cell.metadata.data["('corrected highlights', 'FP(corr)')"]
    ):  # FP
        return "orange"
...
def generate_dg_metrics_results(df_metrics):
    ...
    lower_index = ["TP", "FP", "FN", "precision"] + [
        "TP(corr)",
        "FP(corr)",
        "FN(corr)",
        "precision",
    ...
    renderers = {
        "('corrected highlights', 'TP(corr)')": TextRenderer(
            background_color=Expr(background_color_tp),
            bold=True,
            horizontal_alignment="center",
        ),
        "('corrected highlights', 'FP(corr)')": TextRenderer(
            background_color=Expr(background_color_tp),
            bold=True,
            horizontal_alignment="center",
        ),
        "('corrected highlights', 'FN(corr)')": TextRenderer(
            bold=True, horizontal_alignment="center"
        ),
        "('corrected highlights', 'precision')": TextRenderer(
            bold=True, horizontal_alignment="center"
        ),
        "('raw highlights', 'TP')": TextRenderer(
            background_color=Expr(background_color_tp),
            bold=True,
            horizontal_alignment="center",
        ),
        "('raw highlights', 'FP')": TextRenderer(
            background_color=Expr(background_color_tp),
            bold=True,
            horizontal_alignment="center",
        ),
