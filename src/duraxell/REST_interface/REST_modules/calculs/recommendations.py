    ...
    for filename in txt_files:
        with open(
            ...
            for match in re.finditer(regex, text):
                ...
                place_end = match.end()
                spacing_regex_results.append(
                    [
                        regex,
                        filename,
                        [place_start, place_end],
                        "?",
                        ((place_end - place_start) - constant_dist),
                    ]
                )
...
def compare_spacing_regex_locations(spacing_regex_results, current_entity, df):
    ...
    for (
        ...
        if add_to_tp:
            ...
            updated_spacing_regex_results.append(
                [regex, filename, [place_start, place_end], "FP", distance]
            )
...
def create_fig_recommandation(spacing_regex_values, title):
    ...
    sns.scatterplot(
        data=df,
        x="Distance",
        y=np.random.uniform(low=-0.28, high=0.28, size=len(df)),
        hue="Type",
        palette={"FP": "red", "TP": "blue"},
        marker="o",
        s=50,
        legend=False,
    )
    ...
    if list_spacing_regex:
        ...
        with output_recommendations:
            for _index, spacing_regex in enumerate(list_spacing_regex):
                ...
                spacing_regex_results_compared = compare_spacing_regex_locations(
                    spacing_regex_results, current_entity, df
                )
                df_spacing_regex_results = pd.DataFrame(
                    spacing_regex_results_compared,
                    columns=["regex", "filename", "location", "value", "distance"],
                )
        ...
        return widgets.Accordion(
            ...
            titles=("Recommendations for optimal spacing expressions", "other"),
        )
    else:
