...
def load_from_brat(path, merge_spaced_fragments=True, merge_all_fragments=False):
    ...
    filenames = {
        os.path.relpath(filename, root_path).rsplit(".", 1)[0]: {
            "txt": filename,
            "ann": [],
        }
    ...
    for filename in glob.glob(path.replace(".txt", ".a*"), recursive=True):
        key = os.path.relpath(filename, root_path).rsplit(".", 1)[0]
        if key in filenames:
            filenames[key]["ann"].append(filename)
    for doc_id, files in filenames.items():
        entities = {}
        relations = []
        events = {}

        doc_id = files["txt"].replace(".txt", "").replace("\\", "/").split("/")[-1]
        # doc_id = "JIODAO"
        ...
        if merge_all_fragments:
            ...
            for entity in entities.values():
                ...
                if len(fragments) == 1:
                    ...
                    entity["fragments"] = [{"begin": begin, "end": end}]
                    merged_entities.append(entity)
