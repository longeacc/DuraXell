...
(
    value_button_results,
    current_category,
    current_entity,
    options,
    list_spacing_regex,
    other_categories,
) = initialize_widgets_globals(ent_cat)
(
    button_save,
    button_selection_entity,
    t0,
    t1,
    t2,
    t3,
    button_categorization,
    button_selection_category,
    ban_word_tag,
    space,
    tabs,
) = initialize_widgets(ent_cat, current_entity, ban_words_entities, options)
...
def create_button_results():
    ...
    button_results = widgets.ToggleButton(
        description="Results",
        button_style="info",
        icon="poll",
...
def create_t0():
    ...
    button_accept_file = widgets.Button(
        description="Load",
...
def create_t1_category_selection():
    ...
    """
    button_selection_category.observe(
        lambda change: on_visualization_categorie_change(change), names="value"
    )
    t1_category_selection = widgets.VBox(
        [button_selection_category, output_t1_visualization_category]
    )
...
def create_t1():
    ...
    """
    global t1
    t1 = widgets.Accordion(
        [create_t1_category_selection(), create_t1a2()],
        titles=("Highlights visualization", "Concordancer"),
    )
    return t1
...
def create_frequent_tfidf_texts(df, current_entity, df_tf_results, ban_words_tfidf):
    ...
    for keyword in treated_n_grams:
        ...
        for gram, occurrence in treated_n_grams[keyword].items():
            list_widget.extend(
                [
                    widgets.TagsInput(value=gram),
                    widgets.HTML(value="(" + str(occurrence) + ")"),
                ]
            )
            add = True
        if add:
...
def on_ban_word_tag_change(change):
    ...
    if not change["new"]:
        ...
        ban_words_entities[current_entity] = change["new"]
    button_save.button_style = "info"
    button_categorization.button_style = "warning"
...
def create_categories_tags():
    ...
    button_add_category = widgets.Button(
        button_style="success", icon="plus-circle", layout=widgets.Layout(width="70px")
    )
...
def create_t2a1():
    ...
    t2a1_title2 = widgets.HTML(
        value=f"<h2 style='height: 20px; line-height: 20px; text-align: left; display: flex; align-items: center;'>{title2}</h2>"
    )
    t2a1_tags = create_categories_tags()
    accordion_recommendations = create_accordion_recommendations(
        list_spacing_regex, path, current_entity, df
    )
...
def display_categorization_results():
    ...
    cat_infos = create_categories_infos(df, ent_cat, current_entity).sort_values(
        by="total_annotations_number", ascending=False
    )
...
def categorization():
    ...
    df, other_categories, list_spacing_regex = calculate_categorization(
...
def change_visualization_metric(
    index,
    df_metrics_locations,
    t3_output1_entity_results,
    ...
    if annotation != "no annotation":
        ...
        text2 = text2.replace(
            common_string,
            f'<span style="background-color: orange; padding: 4px 0;">{common_string}</span>',
        )
    ...
    def on_checkbox_isnotfporfn_clicked(change, instruction):
        ...
        if path:
            update_df_result(df_metrics, df, df_metrics_locations)
...
def create_t3a1():
    ...
    t3a2_title = widgets.HTML(
        value=f"<h2 style='height: 30px; line-height: 30px; text-align: left; display: flex; align-items: center;'>{title}</h2>"
    )
    title = "III - Summary table of found pattern"
    t3a3_title = widgets.HTML(
        value=f"<h2 style='height: 30px; line-height: 30px; text-align: left; display: flex; align-items: center;'>{title}</h2>"
    )

    # calculation of metrics dataframes
    metrics_locations = calculate_location_metrics(
        current_entity,
        ent_cat,
        path,
        df,
    ...
    df_metrics_locations = pd.DataFrame(
        metrics_locations[current_entity],
        columns=[
            "category",
            "result",
            "isNotFP",
...
def launch_rest():
    ...
    display(
        HTML(
            "<style>.widget-dropdown select, .widget-dropdown .widget-readout { font-size: 14px !important; }</style>"
        )
    )
    t0 = create_t0()
    ...
    selection_results_save = widgets.HBox(
        [
            button_selection_entity,
            button_results,
            initiate_button_save(button_save, button_results),
        ]
    )
